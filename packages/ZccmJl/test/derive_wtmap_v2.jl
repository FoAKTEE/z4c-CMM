...
# Q_wt local form: Q = r^2 dr U in the corrected gauge
tQ = scal2(at_wt(Z.ang_rpow(Z.ang_dr(U_c), 2)), :sc)
candQ = Dict(
 :dth_trace => scal2(at_wt(Z.ang_dtheta(trace)), :sc),
 :dth_dr_trace => scal2(at_wt(Z.ang_dtheta(Z.ang_dr(trace))), :sc),
 :hrth => scal2(at_wt(h_rth), :sc),
 :dr_hrth => scal2(at_wt(Z.ang_dr(h_rth)), :sc),
 :dth_hrr => scal2(at_wt(Z.ang_dtheta(h_rr)), :sc),
 :dt_hrth => scal2(at_wt(Z.ang_du(h_rth)), :sc),
 :dth_hTT => scal2(at_wt(Z.ang_dtheta(Z.ascale(Z.aadd(h_T,Z.ascale(h_P,-1)),1//2))), :sc),
)
println("Q_wt fit: ", fit2(tQ, candQ, [:dth_trace, :dth_dr_trace, :hrth,
        :dr_hrth, :dth_hrr, :dt_hrth, :dth_hTT]))
# dump the anchored-gauge series for the evolve gate (generated file)
open(joinpath(@__DIR__, "..", "src", "anchored_series.jl"), "w") do f
    println(f, "export J_ANCH, U_ANCH, W_ANCH, BETA_ANCH")
    println(f, "# AUTO-GENERATED by test/derive_wtmap_v2.jl (iter 42):")
    println(f, "# consistent worldtube-anchored gauge series (l=2 m=0,")
    println(f, "# Teukolsky; keys (n,k,p,q) = F^n r^-k c^p s^q).")
    for (nm, ser) in (("J_ANCH", J_c), ("U_ANCH", U_c), ("W_ANCH", W_c),
                      ("BETA_ANCH", beta_c))
        println(f, "const $nm = AngSeries(")
        for ((n,k,p,q),v) in sort(collect(ser))
            println(f, "    ($n, $k, $p, $q) => big($(numerator(v)))//$(denominator(v)),")
        end
        println(f, ")")
    end
end
println("anchored series dumped")
