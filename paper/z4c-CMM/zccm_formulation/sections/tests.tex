...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.74\textwidth]{zccm_x2_structural.pdf}
\caption{$X=2$ error hierarchy from the dedicated eight-GPU campaign,
all runs at the stable configuration (diss.\ $0.1$, $\kappa_1=0.1$,
$\texttt{nghost}=4$, $t=80$). Top: $r\psi_4^{(2,0)}$ at $r=36$ for the
causally disconnected far-boundary reference ($768^3$, box $\pm192$) and
the boundary-$41$ Sommerfeld and Z4c-CCM runs at $h=0.5$ (which track
the reference) together with the $h=0.25$ Sommerfeld run (whose late-time
departure is the truncation effect). Bottom: at fixed resolution the
boundary error $|{\rm run}-{\rm ref}|$ is
$E=1.1\times10^{-2}$, identical for Sommerfeld and CCM; the
matching-term difference $|{\rm CCM}-{\rm Somm}|$ sits at
$8\times10^{-10}$ (peeling); and the truncation estimate
$E_h=|h_{0.5}-h_{0.25}|$ reaches $0.6$ --- the boundary error lies far
below the truncation error and is insensitive to the matching term, the
$X=2$ result of Ref.~\cite{Ma:2023qjn} reproduced with a shared
linear-initial-data protocol. Generator:
\texttt{scripts/plot\_x2\_structural.py}; data:
