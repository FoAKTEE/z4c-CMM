...
RK4, CFL $0.25$, second-order stencils, $\kappa_1=0.02$), with
\eqref{eq:bc-phys} realized at the Cauchy outer boundary as described below.
...
Parameters of Ref.~\cite{Ma:2023qjn}: $r_c=20$, $\tau=2$,
$X\in\{10^{-5}, 2\}$, Cauchy boundary (worldtube analog) at $41$,
$t\in[0,80]$; extraction of $r\psi_4^{(2,0)}$ at $r=36$; grids $164^3$
($h=1/2$) with $132^3$, $104^3$ error-estimate companions; reference run
with boundary at $104$ ($416^3$, same $h$) — causally disconnected, since
contamination reaching $r=36$ by $t=80$ requires boundary data at
$t_b < 12$, where the solution \eqref{eq:teuk-F} is bounded by
$X e^{-[(104-40)/\tau]^2} = X e^{-1024}$. Error norm on the common output
grid:
\begin{equation}
E[\mathrm{run}] =
\frac{\big\| r\psi_4^{(2,0)}[\mathrm{run}] -
 r\psi_4^{(2,0)}[\mathrm{ref}]\big\|_{2}}
{\big\| r\psi_4^{(2,0)}[\mathrm{ref}]\big\|_{2}},
\qquad
E_{h}[\mathrm{run}] =
\frac{\big\| r\psi_4^{(2,0)}\big|_{164^3} -
 r\psi_4^{(2,0)}\big|_{132^3}\big\|_{2}}
{\big\| r\psi_4^{(2,0)}[\mathrm{ref}]\big\|_{2}} .
\label{eq:teuk-E}
\end{equation}
Measured (Fig.~\ref{fig:teuk}; 4$\times$A100, every run $<600$\,s):
\begin{align}
X=10^{-5}&:&
E[\text{Somm}] &= E[\text{CCM}] = 2.732\times10^{-4}
&&\ll\; E_h = 0.70,
\nonumber\\
X=2&:&
E[\text{Somm}] &= E[\text{CCM}] = 8.87\times10^{-2}
&&\ll\; E_h = 0.69,
\label{eq:teuk-numbers}
\end{align}
reproducing the boundary-error $\ll$ truncation-error hierarchy and the
perturbative indistinguishability of Ref.~\cite{Ma:2023qjn} (their
Sec.~V.A). The CCM$-$Sommerfeld waveform difference switches on exactly at
the boundary$\to$extraction causal time ($t \ge 64.5$) at relative
amplitude $5\times10^{-10}$--$4\times10^{-9}$: the self-datum
\eqref{eq:teuk-psi0} obeys peeling, $\psi_0\sim r^{-5}$, so a purely
outgoing pulse supplies an incoming correction suppressed by $41^{-5}$ at
this worldtube — resolving CCM from Sommerfeld at figure level requires
the CCE-fed datum (nonlinear backscatter), the documented successor of the
$\psi_0^{\rm datum}$ provider. The deficit of the measured peak against the
scri formula \eqref{eq:teuk-psi0} is finite-difference truncation, not the
finite extraction radius: peak ratio $0.292/0.465/0.612$ at
$104^3/132^3/164^3$ ($\sim\!4$ points per $\tau$ at second order).
Constraints stay bounded on all runs ($X=2$: $\|H\|_{L_2}$
$3.5\times10^{-3}\to5.4\times10^{-3}$, damped after $t\approx35$, with the
$O(X^2)$ violation of the linear data \eqref{eq:teuk-id} absorbed by the
$\kappa_1$ damping).
\end{nfblock}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.98\textwidth]{zccm_teuk_paper.pdf}
\caption{Exact-parameter Teukolsky test of Sec.~\ref{sec:teuktest}
($r_c{=}20$, $\tau{=}2$, boundary $41$, extraction $r{=}36$; amplitudes
normalized by $X$). Top: $(\ell,m){=}(2,0)$ mode of $r\psi_4$ for the
causally disconnected reference, Sommerfeld, and Z4c-CCM
\eqref{eq:teuk-inject} runs; at $X{=}10^{-5}$ the linear scri formula
\eqref{eq:teuk-psi0} is overlaid (its excess is the second-order truncation
deficit quoted in the text). Bottom: differences \eqref{eq:teuk-E};
$|\mathrm{CCM}-\mathrm{ref}|$ and $|\mathrm{Somm}-\mathrm{ref}|$ coincide,
both sit below the two-resolution error estimates (dashed), and
$|\mathrm{CCM}-\mathrm{Somm}|$ (green) activates only after the
boundary$\to$extraction causal time with the $r^{-5}$ peeling suppression
of the self-datum. Generator:
\texttt{scripts/plot\_paper\_figure\_teuk.py}; data:
\texttt{results/numerical/athenak\_teuk\_paper/} (ledger node N11).}
\label{fig:teuk}
\end{figure}
