...
\subsection{Physical-mode dictionary (node N3)}
\label{sec:n3}

Linear TT perturbation on the frame \eqref{eq:normal},
$\gamma_{AB} = \delta_{AB} + h_{AB}(t,x_s)$,
$h_{22}=-h_{33}\equiv F_1$, $h_{23}\equiv F_2$, $h_{mm} = F_1 + iF_2$;
$\psi_0 \equiv -C_{abcd}\,l^a m^b l^c m^d$ on
$l = \mathring l$, $m = m^A$. Direct evaluation of the linearized Riemann
tensor gives, off shell (no field equations used),
\begin{nfblock}
\begin{equation}
\psi_0 = \tfrac{1}{4}\,(\partial_t + \partial_s)^2\,h_{mm}
\qquad\text{[V2: exact for arbitrary }F_1,F_2\text{]},
\label{eq:psi0lin}
\end{equation}
\begin{equation}
(\partial_t+\partial_s)^2\,\gamma_{AB}^{\rm TF}
 = 4\Big(\psi_0\,\bar m_A \bar m_B + \bar\psi_0\, m_A m_B\Big),
\qquad
(\mathring l\!\cdot\!\partial)^2\gamma_{AB}^{\rm TF}
 = 2\Big(\psi_0'\,\bar m_A \bar m_B + \bar\psi_0'\, m_A m_B\Big)
\label{eq:dictionary}
\end{equation}
\end{nfblock}
[V2: GPU residual $0.0$ over $4{,}194{,}304$ off-shell Fourier modes].
$(\mathring l\!\cdot\!\partial)$ annihilates outgoing waves $f(t-x_s)$:
\eqref{eq:dictionary} is transparent to them.
...
(\mathring l\!\cdot\!\partial)^2\,\gamma^{\rm TF}_{AB}
\;\hat=\;
2\Big(\psi_0'\,\bar m_A\bar m_B + \bar\psi_0'\,m_A m_B\Big),
...
\label{eq:bc-gauge}
\end{align}
Bj{\o}rhus form of \eqref{eq:bc-phys}, with
$V_{AB}\equiv(\mathring l\!\cdot\!\partial)\,\gamma^{\rm TF}_{AB}$:
\begin{equation}
(\mathring l\!\cdot\!\partial)\,V_{AB}\big|_{\partial\Omega}
 = 2\Big(\psi_0'\,\bar m_A\bar m_B + \bar\psi_0'\,m_A m_B\Big);
\qquad
...
Files: \texttt{n2\_boost\_check.txt}, \texttt{n3\_dictionary\_check.txt},
...
V2 & dictionary \eqref{eq:psi0lin}--\eqref{eq:dictionary},
 $c=\tfrac14$, $K=4$ & arbitrary $F_1,F_2$, off shell &
 $0.0$ over $4{,}194{,}304$ off-shell modes \\
...
V6 & 10-mode count, $\psi_0\to0$ reduction, wire-through &
 structural & reduction $0.0$; chain residual $2.92{\times}10^{-16}$ \\
