...
// Vacuum XCTS executable (z4c-CCM mission). Identical to SolveXcts except that
// it omits the HydroQuantitiesCompute observation machinery. The general
// SolveXcts metavariables instantiate HydroQuantitiesCompute for *every*
// registered background via call_with_dynamic_type, which forces each
// background to provide the six hydro primitive variables (RestMassDensity,
// SpecificEnthalpy, Pressure, SpatialVelocity, LorentzFactor, MagneticField).
// A genuine vacuum free-data background such as Xcts::AnalyticData::TeukolskyWave
// has no matter and must never be asked for hydro variables. This executable
// removes the hydro compute tag (and the LowerSpatialFourVelocity observation),
// so vacuum backgrounds need not implement matter variables, and registers
// TeukolskyWave for the arXiv:2308.10361 X=2 Teukolsky test.
...
#include "PointwiseFunctions/AnalyticData/Xcts/DuckTovStar.hpp"
...
struct Metavariables {
  ...
  static constexpr Options::String help{
      "Find the solution to a vacuum XCTS problem (no hydro observation)."};
  ...
  // No hydro_quantities_compute: this is a vacuum solver, so the matter
  // primitives are never retrieved from the background.
  ...
  struct factory_creation
    ...
    using analytic_solutions_and_data = tmpl::push_back<
        ...
        Xcts::AnalyticData::Binary<elliptic::analytic_data::AnalyticSolution,
                                   Xcts::Solutions::all_analytic_solutions>,
        Xcts::AnalyticData::DuckTovStar, Xcts::AnalyticData::TeukolskyWave,
    ...
    using factory_classes = tmpl::map<
        ...
        tmpl::pair<elliptic::analytic_data::AnalyticSolution,
                   tmpl::push_back<Xcts::Solutions::all_analytic_solutions,
                                   Xcts::AnalyticData::DuckTovStar,
                                   Xcts::AnalyticData::TeukolskyWave>>,
        tmpl::pair<
